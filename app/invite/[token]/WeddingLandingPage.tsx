          ...
          events.map((event: EventItem, idx: number) => {
            ...
            return (
              <section
                key={event.id}
                id={`event-${event.id}`}
                className="relative top-[-30px] w-full h-screen overflow-hidden bg-black border-b border-white/10"
              >
                <video
                  ref={event.ref}
                  src={event.video}
                  className="absolute inset-0 top-[-30px] w-full h-full object-cover z-0"
                  playsInline
                  preload="auto"
                  controls={false}
